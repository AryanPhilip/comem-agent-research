...
def execute_pixel_action(responses, page, image_processor=None, observation=None, args=None, retry_config=None):
    """
    ...
    Args:
        ...
        retry_config: Optional RetryConfig — when provided, retryable errors
                      are retried with exponential back-off.

    Returns:
        Tuple of (page_or_done, Optional[ClassifiedError]).
        page_or_done is the Playwright page on success, or "DONE" on stop.
        The error is None on success.
    """
    if isinstance(responses, dict):
        responses = [responses]

    last_action_error = None  # Track the last ClassifiedError across responses
    ...
    for response_id, response in enumerate(responses):
        ...
        try:
            ...
            # Classify the error
            from utils.error_classifier import classify_error as _classify
            page_content = ""
            try:
                page_content = page.content()
            except Exception:
                pass
            classified = _classify(exc, page_content=page_content, page_url=page.url)
            last_action_error = classified
            print(f"[ErrorClassifier] {classified}")

            # If retry_config is provided and error is retryable, attempt retry
            if retry_config is not None:
                from utils.action_retry import action_retry, RetryConfig
                rc = retry_config if isinstance(retry_config, RetryConfig) else RetryConfig(**retry_config)
                if classified.category in rc.retryable_categories:
                    def _retry_fn():
                        # Re-execute just this single response (recursive single-item call without retry)
                        return execute_pixel_action(
                            response, page, image_processor, observation, args,
                            retry_config=None,
                        )
                    result, retry_err = action_retry(_retry_fn, rc, action_description=str(action_type))
                    if retry_err is None and result is not None:
                        # Retry succeeded
                        return result  # already a tuple
                    # Retry exhausted — keep last_action_error and continue
                    if retry_err is not None:
                        last_action_error = retry_err
